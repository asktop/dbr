...
func TestDbrSelect1(t *testing.T) {
    ...
    stmt := Select("id", "name", "value").From("config").Where("id >= ? AND id < ? OR id in ?", 1, 5, ids)
...
func TestDbrSelect2(t *testing.T) {
    ...
    where := And(Gte("id", 1), Lt("id", 5))
    where = Or(where, Eq("id", ids))

    stmt := Select("id", "name", "value", "create_time", "update_time").From("config").Where(where)
...
func TestDbrSelectMaps(t *testing.T) {
    ...
    _, err := Select("id", "name", "value").From("config").Where("id >= ? AND id < ?", 1, 5).Load(&maps)
    if err != nil {
        fmt.Println(err)
    } else {
...
func TestDbrSelectMap(t *testing.T) {
    var maps map[string]string
    _, err := Select("name", "value").From("config").Where("id >= ? AND id < ?", 1, 5).Load(&maps)
    if err != nil {
        fmt.Println(err)
    } else {
...
func TestDbrSelectExpr(t *testing.T) {
    var maps []map[string]string
    count, err := Select("name", "start_time", "msg").From("task_log").
...
func TestDbrSelectJoin(t *testing.T) {
    var maps []map[string]string
    count, err := Select("tl.name", "tl.start_time", "tl.msg").From("task_log tl").
        LeftJoin("task t", "tl.name = t.name").
        Where(Eq("t.id", []int64{2})).
        Load(&maps)
    if err != nil {
        fmt.Println(err)
    } else {
...
func TestDbrSelectBetween(t *testing.T) {
    ids := []int{7, 8, 9}
    rows, err := Select("id", "name", "value").From("config").
        Where(Or(Between("id", 1, 5), Eq("id", ids))).Rows()
    if err != nil {
        fmt.Println(err)
    } else {
        defer rows.Close()
...
func TestDbrInsertMap(t *testing.T) {
    ...
    rs, err := InsertInto("config").Map(kv).Exec()
    if err != nil {
        fmt.Println(err)
    } else {
...
func TestDbrUpdate(t *testing.T) {
    stmt := Update("config").
        Set("value", "value1 u").
        Set("status", Expr("status - ?", "1")). //值为表达式
...
func TestDbrUpdateMap(t *testing.T) {
    setMap := map[string]interface{}{
        "value":       "value2 u",
        "status":      Expr("status + ?", "1"), //值为表达式
        "update_time": time.Now().Unix(),
    }
    rs, err := Update("config").
        SetMap(setMap).
...
func TestDbrCaseUpdate(t *testing.T) {
    stmt := CaseUpdate("user").Columns("name", "age", "info").
        Values("a", Expr("age + ?", 10), "aaa").
        Values("b", Expr("age - ?", 1), "bbb").
        Values("c", 31, "ccc").
        Values("d", 41, "ddd")
...
func TestDbrDelete(t *testing.T) {
    ...
    rs, err := DeleteFrom("config").Where("name in ?", names).Exec()
    if err != nil {
        fmt.Println(err)
    } else {
        fmt.Println(rs.RowsAffected())
...
func TestDbrCache(t *testing.T) {
    ...
    //
    //stmt := Select("user_id", "mobile", "username").From("user").Where(Eq("user_id", []int64{1, 2, 3}))
    //
